...
1. **Application pages**, namely *History*, *Account*, *Transfer*, *Cards*, and *Members*.
...
All are retrieved with the API, though visibility is determined by account membership permissions (an *account membership* is what binds a user to an account).
...
The account information section can also be used to share details about the user's `identificationStatus`, `documentCollectionStatus`, and `transactions`.

![](./images/nav-section2-tags.png)

| Status | Explanation | Tag |
| --- | --- | --- |
| **`identificationStatus`** is `Processing` | Identification verification is still processing | **Pending** |
| **`identificationStatus`** is *not* `ValidIdentity` or `Processing` | Identification verification is at any stage of processing, but not completed | **Action Required** |
| **`identificationStatus`** is `Uninitiated`, `InsufficientDocumentQuality`, `InvalidIdentity`, `ReadyToSign` | If any of these statuses are present **and** the project and account membership [require identity verification](./identity-verification-bypass)  | **Action Required** |
