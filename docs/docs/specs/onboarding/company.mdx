...
:::caution 🇩🇪 Germany
For German companies with German accounts, collect the **company tax number** as well (_Umsatzsteuer-Identifikationsnummer_; 10 or 11 characters, numbers only).
:::
...
The ownership step only applies if the user selected **company**, **association** (specifically _home owner association_), or **“other”** when submitting their preliminary information.
:::
...
- Birth city
- Birth postal code
- Type of control exercised (_Legal representative_, _Ownership of capital_, or _Other_)
  - For _Ownership of capital_ only, add the following:
    - Total percentage of capital held (number)
    - _Directly_ and _Indirectly_ checkboxes to indicate type of ownership
...
- If the owner’s residency country is Germany `DEU`, add:
  - **UBO tax identification number** (_Steuer-Identifikationsnummer_)
